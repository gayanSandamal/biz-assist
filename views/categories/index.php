...
//t1.cat_name as category_name,
//t2.cat_name AS parent_category_name
//FROM category AS t2
//LEFT OUTER JOIN category AS t1 ON t1.parent_cat_id = t2.cat_id
//WHERE t2.cat_name LIKE '%' AND NOT t1.cat_id = 0
...
FROM category AS t2
LEFT OUTER JOIN category AS t1 ON t1.parent_cat_id = t2.cat_id
WHERE NOT t1.cat_id = 0
ORDER BY category_name ASC;
";

$result_view_categories = $conn->query($view_categories);
?>
                        ...
                        <form method="POST" action="delete-multiple.php">
                            ...
                            <div class="card-content table-responsive">
                                <table class="table hover" id="cat-table">
                                    ...
                                    <tbody>
                                        <?php
                                        if ($result_view_categories->num_rows > 0) {
// output data of each row
                                            $numOrder = 1;
                                            while ($row = $result_view_categories->fetch_assoc()) {
                                                ?>
                                                ...
                                                <tr class="product-single-container">
                                                    <th class="product-select">
                                                        <div class="checkbox">
                                                            <label>
                                                                <input type="checkbox" name="check_list[]" value="<?php echo $row["category_id"]; ?>" class="cat-check">
                                                            </label>
                                                        </div>
                                                    </th>
                                                    ...
                                                    <td class="product-actions">
                                                        <a href="edit.php?id=<?php echo $row["category_id"]; ?>" class="product-actions-btn product-actions-edit"><i class="material-icons">mode_edit</i><span>Edit</span></a>
                                                        <a href="<?php echo $row["category_id"]; ?>" class="product-actions-btn product-actions-delete" id="category-actions-delete"><i class="material-icons">delete</i><span>Delete</span></a>
                                                    </td>
                                                </tr>
...
<script>
    ...
    $('a#category-actions-delete').click(function (e) {
        e.preventDefault();
