    ...
    <div class="content">
        <div class="container-fluid no-side-paddings">
            ...
            <div class="shop-container list-view-on">
                <h1 class="is-loading">Loading...</h1>
                <table id="shop-table" class="hide">
                    <thead>
                    ...
                    <tbody>
                        ...
                        ?>
                                <tr id="<?php echo $row["product_id"]; ?>" onclick="add_to_invoice('<?php echo ($row['availability'] == 1 ? '1' : '0'); ?>', '<?php echo $row['product_id']; ?>')" class="<?php echo $row["qty"] <= 0 ? 'out-of-stock' : ''; ?> <?php echo ($row["discounted"] > 0 ? 'discounted' : ''); ?>">
                                    <!-- <td class="item-category-name cat-name-is-<?php //echo $row['cat_name']; 
                                                                                    ?>">
                                    <span><?php //echo $row["cat_name"]; 
                                            ?>&nbsp;</span>
                                </td> -->
                                    <!-- <td class="shop-item-cat text-right"><?php //echo ++$count; 
                                                                                ?>&nbsp;</td> -->
                                    <td class="shop-item-cat"><?php echo $row["cat_name"]; ?></td>
                                    <td class="shop-item-brand"><?php echo $row["brand_name"]; ?></td>
                                    <td class="shop-item-name"><?php echo $row["product_name"]; ?></td>
                                    <td class="shop-item-exp"><?php echo $row["product_exp_date"]; ?></td>
                                    <!-- <td><?php //echo $row["product_model_no"]; 
                                                ?></td> -->
                                    <td class="shop-item-count">
                                        <?php echo $row["qty"] > 0 ? $row["qty"] : '<span class="out-of-stock-label">Out of stock</span>'; ?>
                                    </td>
                                    <!-- <td class="code-tag-number" id="code-<?php //echo $row["product_code_no"]; 
                                                                                ?>">
                                    <?php //echo $row["product_code_no"]; 
                                    ?>
                                </td> -->
                                    <td class="shop-selling-price <?php echo ($row["discounted"] == 0 ? '' : 'discounted'); ?>">
...
<script>
            ...
            "drawCallback": function(settings) {
                ...
                api.column(groupColumn, {
                    ...
                    if (last !== group) {
                        $(rows).eq(i).before(
                            `<tr class="group">
                                <td colspan="6">${group && group.trim() !== '' ? group : 'Uncategorized' }</td>
                            </tr>`
                        );
    ...
    function getItem(item_id) {
        ...
        var cartItem_brand = cartItem.find('.shop-item-brand').text();
        var cartItem_name = cartItem.find('.shop-item-name').text();
        var cartItem_count = cartItem.find('.shop-item-count').text();
        var cartItem_selling_element = cartItem.find('.shop-selling-price');
        var cartItem_hidden_count = parseFloat(cartItem.find('.hidden-count.hide').text());
        if ($(cartItem_selling_element).hasClass('discounted')) {
            cartItem_price = cartItem.find('.shop-discount-value').text().replace('Rs. ', '');
        } else {
            cartItem_price = cartItem.find('.shop-selling-value').text().replace('Rs. ', '');
        }
        var item = {};
        item.id = item_id;
    ...
    function addItem(item) {
        ...
        var append_item = `
            <tr id="` + item.id + `">
                ...
                <td class="text-right discount-td"><div class="invoice-cart-single-ondeal-discount"><input type="number" class="discount-field number text-right" onkeyup="single_discount(` + item.id + `)" onchange="single_discount(` + item.id + `)" value="0"></div></td>
...
`;
        ...
        if (item_tr.length > 1) {
            ...
            item_tr.find('input.invoice-cart-single-count').on('change keyup', function() {
                ...
                var item_single_total = 0
                if (discount_field > 0) {
                    item_single_total = ((item_single_price * item_single_count) / 100) * discount_field;
                } else {
                    item_single_total = item_single_price * item_single_count
                }
                item_tr.find('.invoice-cart-single-total > span').text(parseFloat(item_single_total).toFixed(2));
    ...
    function single_discount(item_id) {
        ...
        if ((single_discount_field.val() === "") || (single_discount_field.val() < 0) || (single_discount_field.val() === 0)) {
            ...
            single_discounted_price = (single_price_cart * single_count_cart) - ((single_price_cart * single_count_cart) / 100) * single_discount_field_val;
        }
    ...
    function calculate() {
        ...
        $('input.discount-field').each(
            function() {
                ...
                let qty = parseInt(row.find('.invoice-cart-single-count').val())
                let price = parseFloat(row.find('.invoice-cart-single-price > span').text())
                let disc = parseFloat(row.find('.discount-field').val())
                discount_total += ((qty * price) / 100) * disc
            }
        );
    ...
    function proceed() {
        if (($('input.tot-price.number').val() === "") || ($('input.tot-price.number').val() === "0")) {
            ...
            var discounted_price = parseFloat($('input.tot-price.number').val());
            //            var dt = new Date();
            ...
            if (customer_id_init == '0') {
                ...
                returning_customer_discounted_price = discounted_price - ((discounted_price / 100) * customer_discount_profit);
                //                returning_customer_discounted_price = 0;
            }
...
<div class="proceed-form-container">
  <div class="proceed-form">
        ...
        <table style="width:100%;font-size:14px;font-weight:400;font-family:Arial;">
          ...
          <tfoot>
            ...
            <tr>
              ...
              <td style="text-align:right;"><span id="sub_discount">Rs. ` + sub_discount + `</span></td>
            </tr>
            <tr>
              <td colspan="3">Discounted price</td>
              <td style="text-align:right;">Rs.<span id="sub_total">` + discounted_price + `</span></td>
            </tr>
    ...
    function record_invoice() {
        ...
        $.post('add-invoice-call.php', {
            invoice_detailsJsonData: invoice_details_obj
        }, function(response) {
