...
FROM tax
";
$result_get_tax = $conn->query($get_tax);
$result_get_tax_to_check = $conn->query($get_tax_to_check);
?>
                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header" data-background-color="purple">
                            <h4 class="title"><a href="<?php echo $baseUrl; ?>/views/tax/" class="back-btn"><i class="material-icons">arrow_back</i></a>Edit Tax</h4>
                            <!--<p class="category">Complete your profile</p>-->
                        </div>
                        <div class="card-content">

                            <?php
                            if ($result_get_tax->num_rows > 0) {
                                while ($row = $result_get_tax->fetch_assoc()) {
                                    ?>
                                    <form class="min-height-form" method="get" action="../../controllers/edit-brands-call.php">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group label-floating">
                                                ...
                                                <input type="text" class="form-control from-display-name name-required required"  name="tax_rate" id="from-display-name" onkeyup="show_matching_cats()" value="<?php echo $row["tax_rate"]; ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group label-floating">
                                                <label class="control-label">Tax description</label>
                                                <input type="text" class="form-control"  name="tax_desc" onkeyup="show_matching_cats()" value="<?php echo $row["tax_desc"]; ?>">
                                            </div>
                                        </div>
                                        <input type="hidden" value="<?php echo $row["tax_id"]; ?>" name="tax_id"/>
                                        <button type="submit" class="btn btn-primary pull-right edit-supplier-button" id="edit-supplier-button">Update</button>
                                        <div class="clearfix">
                                        </div>
                                    </form>
                                    <?php
...
<script>
    ...
    $('#edit-supplier-button').on('click', function (e) {
        ...
        if (name_required.val().length > 0) {
            $.ajax({
                type: 'get',
                url: '../../controllers/edit-tax-call.php',
                data: $('form').serialize(),
